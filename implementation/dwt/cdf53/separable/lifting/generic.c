...
static void NO_TREE_VECTORIZE H_update_unpacked(const TransformStepArguments * tsa)
{
    ...
    if(band_start_x == 0){
        for(size_t y = band_start_y; y < band_end_y; ++y){
            // first 4 columns
            unpacked_kernel(hl, hh, hl, hh, 0.25f, ll, lh);

            ++ll; ++hl; ++lh; ++hh;
            for(size_t x = band_start_x + 1; x < band_end_x; x += 1){ // main area

                unpacked_kernel(hl, hh, hl-1, hh-1, 0.25f, ll, lh);
    ...
    if(band_end_y != band_size_y){
        for(size_t y = band_start_y; y < band_end_y; ++y){
            for(size_t x = band_start_x; x < band_end_x; ++x){
                ...
                unpacked_kernel(ll, hl, ll1, hl1, -0.5f, lh, hh);
        ...
        // last row
        for(size_t x = band_start_x; x < band_end_x; ++x){
            unpacked_kernel(ll, hl, ll, hl, -0.5f, lh, hh);
    ...
    if(band_start_y != 0){
        ...
        for(size_t y = band_start_y; y < band_end_y; ++y){
            for(size_t x = band_start_x; x < band_end_x; ++x){
                ...
                unpacked_kernel(lh, hh, lh1, hh1, 0.25f, ll, hl);
        ...
        // first row
        for(size_t x = band_start_x; x < band_end_x; ++x){
            unpacked_kernel(lh, hh, lh, hh, 0.25f, ll, hl);
            ++ll; ++hl; ++lh; ++hh;
        }
...
TRANSFORM_STEP_FUNC NO_TREE_VECTORIZE cdf53_separable_lifting_generic(size_t step)
{
    switch(step){
        case 0: 
...
void NO_TREE_VECTORIZE cdf53_separable_lifting_generic_transform_tile(const TransformStepArguments * tsa)
{
