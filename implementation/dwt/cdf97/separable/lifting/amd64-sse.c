...
{
    const __m128 ALPHA = _mm_set_ps1(-1.58613434342059f);
...
static inline void FORCE_INLINE H_predict_unpacked_kernel(__m128 R0C0, __m128 R1C0, __m128 R0C1, __m128 R1C1, float * hl, float * hh)
{
    const __m128 ALPHA = _mm_set_ps1(+0.88291107553090f);
    ...
    TMP = load_unpacked_XX01(LL00, R0C1);
    ...
    TMP = load_unpacked_XX01(LH00, R1C1);
...
static inline void FORCE_INLINE H_predict_unpacked_symmetric_extension_kernel(__m128 R0C0, __m128 R1C0, float * hl, float * hh)
{
    const __m128 ALPHA = _mm_set_ps1(+0.88291107553090f);
...
static inline void FORCE_INLINE H_update_unpacked_kernel(__m128 C0, __m128 X0, __m128 C1, __m128 X1, float * ll, float *lh, const float ALPHA)
{
    const __m128 COEF = _mm_set1_ps(ALPHA);

    __m128 R1 = _mm_add_ps(C0, load_unpacked_XX01L(C0, C1));
    __m128 R2 = _mm_add_ps(X0, load_unpacked_XX01L(X0, X1));
#ifdef USE_FMA
    R1 = _mm_fmadd_ps(R1, COEF, _mm_load_ps(ll));
    R2 = _mm_fmadd_ps(R2, COEF, _mm_load_ps(lh));
#else
    R1 = _mm_mul_ps(R1, COEF);
    R1 = _mm_add_ps(R1, _mm_load_ps(ll));
    R2 = _mm_mul_ps(R2, COEF);
    R2 = _mm_add_ps(R2, _mm_load_ps(lh));
#endif
    _mm_store_ps(ll, R1);
    _mm_store_ps(lh, R2);
}

static inline void FORCE_INLINE H_update_unpacked_symmetric_extension_kernel(__m128 C0, __m128 X0, float * ll, float *lh, const float ALPHA)
{
    const __m128 COEF = _mm_set1_ps(ALPHA);

    __m128 R1 = _mm_add_ps(C0, load_unpacked_HX01LS(C0));
    __m128 R2 = _mm_add_ps(X0, load_unpacked_HX01LS(X0));
#ifdef USE_FMA
    R1 = _mm_fmadd_ps(R1, COEF, _mm_load_ps(ll));
    R2 = _mm_fmadd_ps(R2, COEF, _mm_load_ps(lh));
#else
...
static void H_predict_unpacked(const TransformStepArguments * tsa)
{
    ...
    const size_t next_y = band_stride_y - (band_end_x - band_start_x) + 4;
    ...
    if(band_end_x == band_size_x){
        for(size_t y = band_start_y; y < band_end_y; ++y){
            R0C0 = _mm_load_ps(ll); R1C0 = _mm_load_ps(lh);
            for(size_t x = band_start_x; x < band_end_x - 4; x += 4){
                R0C1 = _mm_load_ps(ll+4); R1C1 = _mm_load_ps(lh+4);

                H_predict_unpacked_kernel(R0C0, R1C0, R0C1, R1C1, hl, hh);
            ...
            H_predict_unpacked_symmetric_extension_kernel(R0C0, R1C0, hl, hh);
...
static void H_update_unpacked(const TransformStepArguments * tsa)
{
    ...
    if(band_start_x == 0){
        for(size_t y = band_start_y; y < band_end_y; ++y){
            ...
            H_update_unpacked_symmetric_extension_kernel(C0, X0, ll, lh, -0.05298011857290f);
            ...
            for(size_t x = band_start_x + 4; x < band_end_x; x += 4){
                ...
                H_update_unpacked_kernel(C0, X0, C1, X1, ll, lh, -0.05298011857290f);
...
static void H_update_unpacked_2(const TransformStepArguments * tsa)
{
    const size_t band_stride_y = tsa->tile_bands.stride_y;
    ...
    if(band_start_x == 0){
        for(size_t y = band_start_y; y < band_end_y; ++y){
            ...
            H_update_unpacked_symmetric_extension_kernel(C0, X0, ll, lh, +0.44350685204390f);
            ...
            for(size_t x = band_start_x + 4; x < band_end_x; x += 4){
                ...
                H_update_unpacked_kernel(C0, X0, C1, X1, ll, lh, +0.44350685204390f);
    ...
    if(band_end_y != band_size_y){
        for(size_t y = band_start_y; y < band_end_y; ++y){
            for(size_t x = band_start_x; x < band_end_x; x += 4){
                ...
                V_unpacked_kernel(lh, hh, ll, hl, ll1, hl1, -1.58613434342059f);
        ...
        // last row
        for(size_t x = band_start_x; x < band_end_x; x += 4){
            V_unpacked_kernel(lh, hh, ll, hl, ll, hl, -1.58613434342059f);
...
static void V_predict_unpacked_2(const TransformStepArguments * tsa)
{
    const size_t band_size_y = tsa->tile_bands.size_y;
    const size_t band_stride_y = tsa->tile_bands.stride_y;
    ...
    if(band_end_y != band_size_y){
        for(size_t y = band_start_y; y < band_end_y; ++y){
            for(size_t x = band_start_x; x < band_end_x; x += 4){
                ...
                V_unpacked_kernel(lh, hh, ll, hl, ll1, hl1, +0.88291107553090f);
        ...
        // last row
        for(size_t x = band_start_x; x < band_end_x; x += 4){
            V_unpacked_kernel(lh, hh, ll, hl, ll, hl, +0.88291107553090f);
    ...
    if(band_start_y != 0){
        ...
        for(size_t y = band_start_y; y < band_end_y; ++y){
            for(size_t x = band_start_x; x < band_end_x; x += 4){
                ...
                V_unpacked_kernel(ll, hl, lh, hh, lh1, hh1, -0.05298011857290f);
        ...
        // first row
        for(size_t x = band_start_x; x < band_end_x; x += 4){
            V_unpacked_kernel(ll, hl, lh, hh, lh, hh, -0.05298011857290f);
            ll+=4; hl+=4; lh+=4; hh+=4;
        }
...
static void V_update_unpacked_2(const TransformStepArguments * tsa)
{
    const size_t band_stride_y = tsa->tile_bands.stride_y;
    float * LH = tsa->tile_bands.LH;
    float * HH = tsa->tile_bands.HH;
    ...
    if(band_start_y != 0){
        ...
        for(size_t y = band_start_y; y < band_end_y; ++y){
            for(size_t x = band_start_x; x < band_end_x; x += 4){
                ...
                V_unpacked_kernel(ll, hl, lh, hh, lh1, hh1, +0.44350685204390f);
        ...
        // first row
        for(size_t x = band_start_x; x < band_end_x; x += 4){
            V_unpacked_kernel(ll, hl, lh, hh, lh, hh, +0.44350685204390f);
            ll+=4; hl+=4; lh+=4; hh+=4;
        }
...
TRANSFORM_STEP_FUNC NO_TREE_VECTORIZE cdf97_separable_lifting_amd64_sse(size_t step)
{
    switch(step){
        case 0:
...
void NO_TREE_VECTORIZE cdf97_separable_lifting_amd64_sse_transform_tile(const TransformStepArguments * tsa)
{
